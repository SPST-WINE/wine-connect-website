...
const FALLBACK_CARRIERS: Carrier[] = [
  { code: "UPS", name: "UPS", tracking_url_template: "https://www.ups.com/track?loc=it_IT&tracknum={tracking}", active: true },
  { code: "DHL", name: "DHL", tracking_url_template: "https://www.dhl.com/it-it/home/tracking.html?tracking-id={tracking}", active: true },
  { code: "FEDEX", name: "FedEx", tracking_url_template: "https://www.fedex.com/fedextrack/?trknbr={tracking}", active: true },
  { code: "TNT", name: "TNT", tracking_url_template: "https://www.tnt.com/express/it_it/site/shipping-tools/tracking.html?searchType=CON&cons={tracking}", active: true },
  { code: "GLS", name: "GLS", tracking_url_template: "https://www.gls-italy.com/it/servizi-online/ricerca-spedizioni?match={tracking}", active: true },
  { code: "OTHER", name: "Altro corriere", tracking_url_template: null, active: true },
];
...
function buildTrackingUrl(code?: string | null, template?: string | null) {
  if (!code || !template) return null;
  return template.replace(/\{tracking\}/g, encodeURIComponent(code));
}
...
export default async function AdminOrderDetail({ params }: { params: { id: string } }) {
  ...
  const { data: order, error } = await supa
    .from("orders")
    .select(`
      id, buyer_id, type, status, created_at, tracking_code, carrier_code,
  ...
  // Corrieri (niente filtro su active per evitare sorprese RLS; filtriamo lato app)
  const { data: carriersDB } = await supa
    .from("shipping_carriers")
    .select("code,name,tracking_url_template,active")
    .order("name", { ascending: true }) as unknown as { data: Carrier[] | null };

  const carriers: Carrier[] =
    (carriersDB?.filter(c => c.active ?? true) || []).length > 0
      ? (carriersDB as Carrier[]).filter(c => c.active ?? true)
      : FALLBACK_CARRIERS;

  const selectedCarrier = carriers.find(c => c.code === order.carrier_code) || null;
  const trackingHref = buildTrackingUrl(order.tracking_code, selectedCarrier?.tracking_url_template ?? null);
      ...
      <div className="mx-auto max-w-6xl py-6 space-y-8">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-xs uppercase tracking-wider text-white/60">Admin · Order</div>
        ...
        <section className="grid gap-4 md:grid-cols-[1fr_320px]">
          <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-5">
            <div className="text-xs uppercase tracking-wider text-white/60">Status & Tracking</div>

            <form
              action="/api/admin/orders/update"
              method="post"
              className="mt-3 grid gap-3 lg:grid-cols-[220px_1fr_260px_auto]"
            >
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-5">
            ...
            <div className="mt-2 text-2xl font-bold text-white">{money(subtotal)}</div>
            <div className="text-xs text-white/60 mt-1">(item × qty, tasse/sped. non incluse)</div>
          </div>
        </section>
