...
export default function Login() {
  ...
  async function submit(e:React.FormEvent){
    ...
    const res = mode==="login"
      ...
      : await supa.auth.signUp({ email, password, options:{ data:{ company_name:"", country:"", name:"" } }});
    setLoading(false);
    if(!res.error) r.push("/catalog"); else alert(res.error.message);
  }

  return (
    <div className="max-w-sm mx-auto mt-16">
      <h1 className="text-2xl font-semibold mb-4">{mode==="login"?"Login":"Crea account"}</h1>
