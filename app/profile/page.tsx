...
export default async function ProfilePage() {
  ...
  const { data: buyer } = await supa.from("buyers")
  ...
  const { data: addresses } = await supa.from("addresses")
    ...
    .eq("buyer_id", buyer.id).order("is_default", { ascending: false });

  const { data: compl } = await supa.from("compliance_records")
    .select("id, mode, documents").eq("buyer_id", buyer.id).maybeSingle();

  return (
    <div className="max-w-3xl mx-auto space-y-10">
      <header className="space-y-1">
        <h1 className="text-2xl font-semibold">Profile</h1>
        <p className="text-sm text-neutral-600">{buyer.company_name} — {buyer.email}</p>
      </header>
