...
/* Preferisci SVG con fallback PNG: metti i file in /public */
...
export default function WineConnectDesktop() {
  ...
  // Fix TS per custom property CSS: '--wc'
  const rootStyle = {
    '--wc': WC_PINK,
    background:
    ...
    <main className="font-sans text-slate-100 selection:bg-[color:var(--wc)]/30" style={rootStyle}>
      ...
      <section className="relative overflow-hidden py-12">
        ...
        <div className="mx-auto max-w-[1200px] px-5">
          ...
          {/* Split Buyer / Wineries con logo centrale */}
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-8 items-center">
              ...
              <ul className="mt-3 space-y-2 text-[15px] text-white/80">
                {t.buyers.points.map((p, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <span
                      aria-hidden
                      className="mt-2 inline-block h-1.5 w-1.5 rounded-full bg-[color:var(--wc)]"
                    />
                    <span>{p}</span>
                  </li>
                ))}
            ...
            {/* RIGHT (wineries) — testi allineati a destra, pallino DOPO */}
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
